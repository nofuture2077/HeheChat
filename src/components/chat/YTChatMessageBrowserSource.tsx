    ...
    return (
        <div className={classes.msg}>
            <span className={classes.badges}>
                <IconBrandYoutube size={config.fontSize * 1.2} color="#FF0000" style={{ verticalAlign: 'text-bottom', marginRight: '2px' }} />
                {msg.isOwner && (
                    <IconCrown size={config.fontSize * 1.2} color="#FFD700" style={{ verticalAlign: 'text-bottom', marginRight: '2px' }} title="Channel Owner" />
                )}
                {msg.isModerator && !msg.isOwner && (
                    <IconShieldFilled size={config.fontSize * 1.2} color="#5E84F1" style={{ verticalAlign: 'text-bottom', marginRight: '2px' }} title="Moderator" />
                )}
                {msg.isMembership && (
                    <span style={{ fontSize: '0.75em', backgroundColor: '#0F9D58', color: 'white', padding: '2px 6px', borderRadius: '3px', marginRight: '4px', verticalAlign: 'text-bottom', fontWeight: 'bold' }}>
                        MEMBER
                    </span>
                )}
                {msg.isVerified && (
                    <IconShieldCheckFilled size={config.fontSize * 1.2} color="#606060" style={{ verticalAlign: 'text-bottom', marginRight: '2px' }} title="Verified" />
                )}
            </span>
