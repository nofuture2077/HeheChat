...
import PallyggLogo from '@/res/pally_logo.svg?react';
...
export function ConnectStatusSettings() {
  ...
  const reinitialize = async (apiKey: string, label: string) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/reconnect?token=${token}&service=${apiKey}`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.success) {
  ...
  const reinitializeAll = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/api/reconnect?token=${token}&service=all`, {
        method: 'POST',
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <>
      ...
      >
        <Text size="sm" mb="lg">
          This will reconnect all configured services. Ongoing events may be interrupted briefly.
          Continue?
        </Text>
        <Group justify="flex-end" gap="sm">
          ...
          <Button color="red" onClick={reinitializeAll}>
            Reinitialize All
          </Button>
        </Group>
      </Modal>
                        ...
                        {typeof LOGOS[key] === 'string' ? (
                          <img
                            ...
                            style={{
                              ...
                              height: 24,
                              objectFit: 'contain',
                              color: 'var(--mantine-color-text)', // This will act as currentColor
